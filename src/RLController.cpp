...
bool RLController::run()
{
  ...
  {
    // mc_rtc::log::info("PosW Sim z: {}",robot().bodySensor("FloatingBase").position().z());
    double pos_z = robot().bodySensor("FloatingBase").position().z();
    if(pos_z < 0.6)
    {
      // lol
      mc_rtc::log::info("The robot falls at {}", experimentDuration);
      return false;
    }
  }
